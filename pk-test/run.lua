...
local run_tests_in_path = function(
  ...
  -- parse test names, if any specified
  local test_names = false
  do
    local parts = split_by_char(test_case_filter, ":")
    if #parts == 2 then
      test_case_filter = parts[1]
      test_names = split_by_char(parts[2], ",")
    end
  end
...
Examples:
    ...
    pk-test "0099"               -- all tests from the case
    pk-test "0099:test1,test2"   -- only test1 and test2 from the case
