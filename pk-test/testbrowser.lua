...
do
  ...
  local GET = function(self, url, ...)
    -- vararg exists here because of a need for a
    -- backward compability and to unify methods
    local nargs = select('#', ...)
    local request_headers = { }

    if nargs == 1 then
      request_headers = select(1, ...)
    elseif nargs == 2 then
      request_headers = select(2, ...)
    else
      error(
        'Wrong number of arguments to GET request. Expected: 2 or 3, Actual: '
        .. nargs
      )
    end
  ...
  -- issue PUT request
  local PUT = function(self, url, request_body, request_headers)
    request_body = request_body or ""
    request_headers = request_headers or { }
    method_arguments(self,
        "string", url,
    ...
    perform(self, "PUT", url, request_body, request_headers)
    return self.code
  end

  -- issue DELETE request
  local DELETE = function(self, url, request_body, request_headers)
    request_body = request_body or ""
    request_headers = request_headers or { }
    method_arguments(self,
        "string", url,
    ...
    perform(self, "DELETE", url, request_body, request_headers)
    return self.code
  end
